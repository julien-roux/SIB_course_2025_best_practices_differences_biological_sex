...
```{r}
## Load the prepared RangedSummarizedExperiment object and convert to DESeqDataSet
rse <- readRDS("rse_SRP218156.rds")
...
![](img/round-help-button.png) What do you observe?
...
![](img/round-help-button.png) What do you observe? What is going on for some genes? (e.g., *Gm20775*)

![](img/elemental-tip.png) Bigwig files are available through recount3 for some of the samples and allow to visualize the read coverage (for example on the Y-chromosome) on a genome browser. For example you can provide these links to the UCSC genome browser directly:
...
## Which sample metadata associate with which PCs?
...
![](img/round-help-button.png) To which principal components contribute which factors? Does this corresponds to the expectations? What do you conclude on the relative effect size of different factors?

```{r}
## Visualize how some of the factors separate on the PCA, for example sex is associated with PC3
plotPCA(vst, 
        ntop = 500,
...
## For visualization: since we account for tissue in the model, it makes sense to correct the expression values for this effect
...
![](img/round-help-button.png) Do the number of DE genes correspond to your expectations from the PCA? What would you conclude on the difference of Tamoxifen effects in male and female? Are some of these genes described in the original paper?

```{r}
## See Fig 4e and f
data.frame(res[rowData(dds)$gene_name %in% c("Egr2", "Fos", "Dusp1", "Nr4a1", "Sik1", "Arc", "Egr1", "Plcl2", "Galnt9", "Per2", "Zbtb16", "Map3k13", "Banp"), ]) |> as_tibble() |> arrange(padj)
```

## Extracting the interaction term

Although this is not tested explicitely, a main aim of the study is to identify genes reacting differently to Tamoxifen treatment in male and female. It is possible to test this by extracting the interaction between sex and tratment.

See `?DESeq2::results` explaining how to extract this effect (Example 3). We need to update the design in the DESeq2 object.

```{r}
design(dds) <- ~ tissue + treatment * sex
dds <- DESeq(dds)
resultsNames(dds)
## Select the last coefficient
...
![](img/round-help-button.png) What do you think about these results? Are these genes described in the original paper? How would you describe the behavior of these genes in simple words?

# GSEA

Although we detect few significant genes for the interaction, it is possible that some trends are visible "collectively" at the gene set level
...
head(fgseaRes[order(fgseaRes$pval), ], n=11)
```
